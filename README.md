...
Two functions exist so far, and they are there mainly so that a broken native
build fails a test rather than being discovered later:
...
`read_xlsx()`, the cell reader and the column builders are not written yet.
The design they will follow is in `.agents/design-zuxlsx.md`.
